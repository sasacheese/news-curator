...
当月ぶんだけを読み、何年ぶん貯まっても 1 回の転送量は変わりません。
...
読めたぶんから結果に反映します。全部揃うのを待たないので、古い月が増えても最初の結果はすぐ出ます。
...
転送ゼロで再利用しています。最新ぶんだけ毎回検証します。履歴を作り直したときは
...
| 状態 | 到達時期の目安 | すること |
| --- | --- | --- |
| 1 ヶ月ぶんの index が 1MB を超える | 記事数が 2 倍以上に増えたら | 検索用に `summary` を落とした軽量インデックスを別に作る |
