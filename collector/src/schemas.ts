...
const describeItemShape = {
  ...
  takeaways: z
    .array(z.string())
    .describe(
      'この記事が何を言っているのかを、誰が読んでも分かる言葉で書いた3行。ちょうど3行。専門用語・略語・API名・オプション名・設定名は使わない（製品名は主題の特定に必要な1〜2個まで、それ以外は普通の言葉に言い換える）。1行30〜60字、主語述語のある文にする。読者を主語にしない（「〜するなら」で始めない、「参考になる」「材料になる」で終わらない）。数字は残す。',
    ),
...
export const KnowDeepDiveSchema = z.object({
  ...
  impact: z
    .array(z.string())
    .describe(
      '誰の・どの構成に効くか。**1項目1行・40字以内。2〜3個。** 対象バージョン・対象環境・条件だけを短く置く。説明も理由も書かない（「〜なので」「〜のため」が入ったら長すぎる）。「広く影響する」のような曖昧な書き方は禁止。特定できないなら「範囲不明」と書く。',
    ),
  timeline: z
    .array(z.string())
    .describe(
...
export const BuildDeepDiveSchema = z.object({
  ...
  unlocks: z
    .array(z.string())
    .describe(
      'できるようになること。**いちばん重要なものだけに絞る。1〜2個、各60字以内。**「これまで◯◯だったのが、これから△△できる」の差分の形で書く。この形にすれば「何が変わるか」も同時に言えるので項目を分けない。「速くなる」「便利になる」は程度の改善であって、できるようになることではない。差分が無いなら、何が楽になるのかを1つだけ率直に書く。付随的な機能を並べない。',
    ),
  howToTry: z
    .array(z.string())
    .describe(
  ...
  fitFor: z
    .array(z.string())
    .describe(
      '**読者が自分に当てはまるかを YES / NO で答えられる条件**を書く。1〜3個、各50字以内。人物像ではなく、観測できる状態で書くこと。✗「効率化したい開発者に向いている」（誰にでも当てはまり判定できない）/ ✓「CI のビルドが 5 分を超えているなら効く」/ ✓「TypeScript で書いた CLI を配布しているなら効く」。読者が自分の環境を見て即答できる形になっているかを、書いたあとに必ず確認する。',
    ),
  notFor: z
    .array(z.string())
    .describe(
      '**当てはまったら読まなくてよい条件**を書く。1〜3個、各50字以内。fitFor と同じく観測できる状態で書く。✗「本格的な用途には向かない」（判定できない）/ ✓「Windows で動かすなら対象外（未対応）」/ ✓「月 1 万リクエスト未満なら手作業のままでよい」。新しい道具ほど適用範囲は狭いが、そこは書かれないことが多い。記事から読み取れる範囲で書き、推測で広げない。',
    ),
  caveats: z
    .array(z.string())
    .describe(
      '**本当に注意すべきことだけ。0〜2個、各60字以内。** 次のどれかに当たるものだけ書く: (1) 知らずに始めると詰まる前提（別ツールの導入、対応していない環境）(2) お金がかかる (3) データや既存の設定が壊れる。「まだ新しいので注意」「本番利用は慎重に」のような、どの新しい道具にも言える一般論は書かない。該当が無ければ空配列。',
    ),
});
