...
const describeItemShape = {
   ...
   * 見出しに出す日本語のタイトル。
   *
   * カードの見出しは元記事のタイトルをそのまま使うので、英語の記事はそこだけ
   * 英語のまま残っていた（GitHub のリポジトリは「owner/repo — English description」
   * の形なので特に読めない）。原題が日本語のときに書き換えると、正確な題を
   * 要約で劣化させるだけなので、その場合は null を返させる。
   */
  titleJa: z
    .string()
    .nullable()
    .describe(
      '見出しに出す日本語のタイトル。**元タイトルに仮名（ひらがな・カタカナ）が1文字も無いときは必ず書く**' +
        '（英語・中国語・韓国語すべて対象）。**仮名があるなら null**（原題が一番正確な題なので書き換えない）。' +
        '**1行・40字以内。句点で終わらせない。体言止めでよい。**' +
        '直訳ではなく「何の話か」が分かる見出しにする。' +
        'GitHub のリポジトリなら「リポジトリ名 — 何をするものか」の形にする（owner 名は落とす）。' +
        '✓「northcinder — 買い物を代行する AI エージェントの土台」' +
        '✗「Buyer-run, ad-neutral shopping-agent MCP software」（英語のまま）' +
        '✗「購入者が実行する広告中立のショッピングエージェント MCP ソフトウェア」（直訳で何をするものか分からない）' +
        '製品名・リポジトリ名・組織名は原語のまま残す（訳すと検索できなくなる）。' +
        'バージョン番号や数字は落とさない。「必見」「話題の」のような煽りと評価は書かない。',
    ),
...
export const BuildDeepDiveSchema = z.object({
  ...
  tryPrompt: z
    ...
    .nullable()
    .describe(
      '「試し方」を、そのままコーディングエージェント（Claude Code など）に貼って試し始められるプロンプトの形で書いたもの。読者が自分の端末かクラウドの Linux 環境に貼る。' +
        '次の 4 節を、この見出し・この順で必ず書く（見出しは行頭に `# ` を付ける）: `# 試すこと`（1 行の目標。次の行に元の URL）/ `# 手順`（番号付き。1 行 1 コマンドか 1 動作。コマンドはそのまま打てる形。人間しかできない手順は末尾に「（手動）」）/ `# 確認したいこと`（やってみないと分からないことだけ、1〜3 個の箇条書き。「何ができるツールか」は記事に書いてあるので入れない）/ `# 前提・注意`（必要な環境や鍵、記事本文でコマンドが欠けているなどの注意。無ければ「- 特になし」）。' +
