 ...
 *
 * 朝のダイジェスト（send-push.ts）とサンドボックスの結果（trials.ts）で共用する。
 * 送り先も失効の掃除も同じなので、違うのは payload だけ——ここを 2 か所に持つと、
 * 片方だけ失効した購読を消し続けることになる。
 *
...
export interface PushPayload {
  ...
  url: string | null;
  /**
   * 同じ tag の通知は端末上で置き換わる。日次とサンドボックスで別の tag を使う
   * ——同じにすると、朝のダイジェストの通知が試した結果で消える。
   */
  tag: string;
  /** 秒。この時間内に届かなければ捨てさせる */
  ttlSeconds: number;
